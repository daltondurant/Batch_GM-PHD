...
%**************************************************************************
% Batch Markov Chain Monte Carlo Gaussian Mixture Probability Hypothesis 
% Density filter. It uses Markov Chain Monte Carlo to batch process 
% measurements into Gaussian mixtures in the state space.
%
% SOURCE: 
% 
...
%
    ...
    vareps_c = 10;  % clutter intensity decay parameter, =1 standard scaling, <1 slower decay, >1 faster decay
    ...
    est.A = NaN(meas.K,1);   % MCMC average acceptance rate
    
    model.name  = 'MCMC GM-PHD';
    model.stream = stream;
        ...
        if any(~cellfun(@isempty, Zk_cell))
            ...
            for ii = 1:length(Zk_cell)
                ...
                if ~isempty(Zk_cell{ii})
                    ...
                    P_proposal = model.P_birth;
                    % batch process
                    [Zii_ss, Rii_ss, Aii] = hmc(cfig, model, Zk_cell{ii}, m_proposal, P_proposal); % single chain
                    % store
